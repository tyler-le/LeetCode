class Solution:
    def lengthOfLIS(self, nums: List[int]) -> int:
        ...
        As we go thru the inner loop from j = 0...i, if nums[i] > nums[j] then we can take the answer for dp[j] and append nums[i] to that sequence, hence increasing the sequence length by 1 (dp[j] + 1). Now we have a sequence that ends with nums[i] and can choose to update dp[i]
